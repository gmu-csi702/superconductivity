# Name: dosplot.gnu
# Objective: plot density of states 

set term postscript eps size 10.0,7.0 enhanced color font 'Helvetica,20' linewidth 2
set output "dosdatTeH.eps"


EF =system("head -1 ferv_omega | awk '{print }'")
EF = EF+0


set title "Electronic Density of States"
set xlabel "Energy(Ry)"
set ylabel "States/Ry/Atom"
set xrange [-2.0:1.5]

set key left top Right noreverse box linetype -2 linewidth 1.000 samplen 4 spacing 1 width 0
set nolabel
set label 1 "{/Symbol e}_F" at 0.0, 3, 0 left norotate
set noarrow
set arrow 1 from 0.0, graph 0, 0 to 0.0, graph 1, 0  nohead linetype 3 linewidth 1.000

set encoding default

set label 2 "TeH_3" at graph 0.9, 0.9
#Plot total DOS and its angular momentum decompositions: Te
plot "dosdat.bin.plot" u ($2-EF):4 t "Total DOS" w l,\
     "dosdat.bin.plot" u ($2-EF):5 t "DOS--s_{Te}" w l,\
     "dosdat.bin.plot" u ($2-EF):6 t "DOS--p_{Te}" w l, \
     "dosdat.bin.plot" u ($2-EF):7 t "DOS--d_{Te}" w l,\
     "dosdat.bin.plot" u ($2-EF):10 t "DOS--s_{H}" w l dt 2,\
     "dosdat.bin.plot" u ($2-EF):11 t "DOS--p_{H}" w l dt 4




