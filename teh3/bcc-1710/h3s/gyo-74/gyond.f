...
C     DIMENSION TITLE (10)
      CHARACTER*40 TITLE
      DIMENSION RDEN(4),spdeta(3),ETA(3),xmomegasq(3)
...
c     OMSQA=(theta)**2/2.0
 ...
 2    CONTINUE
      WRITE(20,82) TITLE,ETA(II)
      WRITE(21,83) TITLE,EF,NEF,ETA(II)
 82   FORMAT(A40,F10.5)
 83   FORMAT(A40,3F10.5)
C     OMG1(II)=OMEGA1
      CV=5.6466434E+05
      OMEGA(II)=OMEGA(II)/CV
        ALFASQ(II)=ETA(II)/(2.0*AM(II)*OMEGA(II))
C     WRITE(8,300)ALFAS
...
c     OMELOG=1500.0
      ...
      xmomegasq(1)=ETA(1)/EL(1)   
      xmomegasq(2)=ETA(2)/EL(2)   
      WRITE(8,31) xmomegasq(1),xmomegasq(2)
  31  FORMAT (1H ,10X,9Hmomegasq=,2F8.4)
...
C     WRITE(8,999) OMELOG
 999  FORMAT (1H ,10X,8HOMELOG =,F10.4)
      ...
      F1=(1.+(elamd/(2.46+9.35*must))**1.5)**(1./3.)
      TC=F1*OMEGALOG*EXP(RATIO)/1.20
c     TCBCS=1.13*THETA*EXP(-1./elamd)
      WRITE (8,40) TC
c     WRITE (8,42) theta
      WRITE (8,43) must
      WRITE (8,44) F1
